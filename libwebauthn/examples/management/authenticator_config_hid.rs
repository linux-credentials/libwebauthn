    ...
    for mut device in devices {
        ...
        let new_pin_length = if matches!(options[idx], Operation::SetMinPinLength(..)) {
            loop {
                ...
                io::stdout().flush().expect("Failed to flush stdout!");
                let input: String = read!("{}\n");
                if let Ok(l) = input.trim().parse::<u64>() {
                    break l;
                }
            }
        } else {
        ...
        let result = match options[idx] {
            Operation::ToggleAlwaysUv => {
                retry_user_errors!(channel.toggle_always_uv(TIMEOUT)).map(|_| ())
            }
            Operation::SetMinPinLengthRpids => retry_user_errors!(
                channel.set_min_pin_length_rpids(min_pin_length_rpids.clone(), TIMEOUT)
            )
            .map(|_| ()),
            Operation::SetMinPinLength(..) => {
                retry_user_errors!(channel.set_min_pin_length(new_pin_length, TIMEOUT)).map(|_| ())
            }
            Operation::EnableEnterpriseAttestation => {
                retry_user_errors!(channel.enable_enterprise_attestation(TIMEOUT)).map(|_| ())
            }
            Operation::EnableForceChangePin => {
                retry_user_errors!(channel.force_change_pin(true, TIMEOUT)).map(|_| ())
            }
            Operation::DisableForceChangePin => {
                retry_user_errors!(channel.force_change_pin(false, TIMEOUT)).map(|_| ())
            }
        };
        result.unwrap();
