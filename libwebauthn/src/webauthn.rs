    ...
    if C::supports_preflight() {
        if let Some(exclude_list) = &op.exclude {
            let filtered_exclude_list =
                ctap2_preflight(channel, exclude_list, &op.client_data_hash(), &op.relying_party.id).await;
            ctap2_request.exclude = Some(filtered_exclude_list);
        }
    }
    let response = loop {
        let uv_auth_used =
            user_verification(channel, op.user_verification, &mut ctap2_request, op.timeout)
                .await?;
        ...
        {
            channel.send_ux_update(UvUpdate::PresenceRequired.into()).await;
        }
        handle_errors!(
            channel,
            channel.ctap2_make_credential(&ctap2_request, op.timeout).await,
            uv_auth_used,
            op.timeout
        )
    }?;
...
async fn make_credential_u2f<C: Channel>(
    ...
    channel.ctap1_register(&register_request)
        .await?
        .try_upgrade(op)
}
...
async fn get_assertion_fido2<C: Channel>(
    ...
    if C::supports_preflight() {
        let filtered_allow_list =
            ctap2_preflight(channel, &op.allow, &op.client_data_hash(), &op.relying_party_id).await;
        if filtered_allow_list.is_empty() && !op.allow.is_empty() {
            ...
            channel.send_ux_update(UvUpdate::PresenceRequired.into()).await;
            let _ = channel.ctap2_make_credential(&dummy_request, op.timeout).await;
    ...
    let response = loop {
        ...
        handle_errors!(
            channel,
            channel.ctap2_get_assertion(&ctap2_request, op.timeout).await,
            uv_auth_used,
            op.timeout
        )
    }?;
...
#[instrument(skip_all)]
async fn negotiate_protocol<C: Channel>(channel: &mut C, allow_u2f: bool) -> Result<FidoProtocol, Error> {
